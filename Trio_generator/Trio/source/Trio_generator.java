...
public class Trio_generator extends PApplet {
...
public Point getCoordinatesForIndex(int i) {
...
public int getIndexForPoint(Point p) {
...
public void draw() { 
  if (draw) {
    ...
    for (int i=0; i< rows; i++) {
      for (int e=0; e< columns; e++) {
        ...
        if (clicked_box.contains(i*columns+e)) {
          ...
          if (clicked_box.size()==3) {
            float xcord=column_width*1.3f-column_width/1.2f+X_offset/2+column_width/2;
    ...
    if (labeled==2) {
      text("y", column_width*(columns+site_distance)+X_offset+column_width/2.5f+column_width/3, column_height*(0+site_distance/2)+column_height/2); 
      text("x", column_width*(0+site_distance)+X_offset+column_width/2.5f, column_height*site_distance/2-column_height/3-column_height/3);
    }
    for (int i=0; i<rows; i++) {
      ArrayList<Character> abc = init_abc_list();
      if (labeled==1 && rows<=abc.size()) {
        text(abc.get(i), column_width*(columns+site_distance)+X_offset+column_width/2.5f, column_height*(i+site_distance/2)+column_height/2);
      } else {
        text(i+1, column_width*(columns+site_distance)+X_offset+column_width/2.5f, column_height*(i+site_distance/2)+column_height/2);
      }
    }
...
public void initButtons(boolean init) {
  ...
  fh = (int)Math.round((float)column_height/2);
  if (init) {
    buttons.add(new button(fx, fy, fw, fh, "-", 0xffFFFFFF, 0xff00FF00));
  } else {
    if (rows<=5) {
      buttons.get(1).update(fx, fy, fw, fh, 0xffFF0000);
    } else {
      buttons.get(1).update(fx, fy, fw, fh, 0xff00FF00);
    }
  }
  ...
  if (init) {
    ...
    if (rows>=max_columns) {
      buttons.get(2).update(fx, fy, fw, fh, 0xffFF0000);
    } else {
      buttons.get(2).update(fx, fy, fw, fh, 0xff00FF00);
    }
  }
  ...
  if (init) {
    ...
    if (columns<=min_columns) {
      buttons.get(3).update(fx, fy, fw, fh, 0xffFF0000);
    } else {
      buttons.get(3).update(fx, fy, fw, fh, 0xff00FF00);
    }
  }
  ...
  if (init) {
    ...
    if (columns>=max_columns) {
      buttons.get(4).update(fx, fy, fw, fh, 0xffFF0000);
    } else {
      buttons.get(4).update(fx, fy, fw, fh, 0xff00FF00);
    }
  }

  //reset_button
  fx = (int)Math.round((float)column_width-(float)column_width/1.5f+(float)X_offset/2);
  fy = (int)Math.round((float)height-(float)column_height/1.5f);
  fw = (int)Math.round((float)column_width*1.3f);
  fh = (int)Math.round((float)column_height/2);
  if (init) {
  ...
  //random number + field
  fx = (int)Math.round((float)column_width-(float)column_width/1.16f+(float)X_offset/2);
  ...
  ts = (float)Math.floor(((float)((column_height+column_width)/2)*0.2f)*3.6f);
  ...
  if (init) {
    buttons.add(new button(fx, fy, fw, fh, current_random_numb+"", tc, 0xffFFFFFF, ts));
  } else {
...
class button {
  ...
  private int x, y, w, h;
  private String text = "";
  private int tc = 0xffFFFFFF;
  private int mc = 0xffFF0000;//tc=textColor, mc=MainColor
  private float ts = -1;
  ...
  public button(int x, int y, int w, int h, String text, int tc, int mc) {
    this(x, y, w, h, text);
    this.tc = tc;
    this.mc = mc;
  }

  public button(int x, int y, int w, int h, String text, int mc) {
    this(x, y, w, h, text);
    this.mc = mc;
  }

  public button(int x, int y, int w, int h, String text, int tc, int mc, float ts) {
  ...
  public void update(int x, int y, int w, int h, int tc, int mc) {
    update(x, y, w, h, mc);
    this.tc = tc;
  }

  public void update(int x, int y, int w, int h, int tc, String text, int mc) {
    update(x, y, w, h, tc, mc);
    this.text = text;
  }

  public void update(int x, int y, int w, int h, int mc) {
    update(x, y, w, h);
    this.mc = mc;
  }

  public void update(int x, int y, int w, int h, String text, float ts, int tc) {
    update(x, y, w, h);
    this.text = text;
    this.ts = ts;
    this.tc = tc;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Trio_generator" };
    if (passedArgs != null) {
